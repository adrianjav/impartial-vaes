...
from src.mtl import pcgrad, graddrop, NSGD, mgda_ub, GradNorm, imtl_G
...
class VAE(pl.LightningModule):
    def __init__(self, prob_model, latent_size, hidden_size, dropout):
        ...
        # Encoder
        self.encoder = Encoder(prob_model, latent_size, hidden_size, dropout)
        ...
        self.heads = nn.ModuleList([VAEHead(dist, hidden_size) for dist in prob_model])
    ...
    def mtl_parameters(self):
        ...
        yield self._imtl_l[0]

        for comp in self._gradnorm.keys():
            for p in self._gradnorm[comp].parameters():
                yield p
