...
class Metric(with_metaclass(ABCMeta, object)):
    ...
    def started(self, engine):
        ...
        if not hasattr(engine.state, 'temp'):
            engine.state.temp = dict()
        engine.state.temp.update(dict.fromkeys(self.keys, self.reset()))
    ...
    def iteration_completed(self, engine):
        ...
        for i, v in engine.state.registers.items():
            if i in self.keys:
                engine.state.temp[i] = self.update(engine.state.temp[i], v)

    def completed(self, engine):
        for i in self.keys:
            result = self.compute(engine.state.temp[i])
            if torch.is_tensor(result) and len(result.shape) == 0:
                result = result.item()
            engine.state.metrics[i] = result

    def attach(self, engine, handler=None, *args, vars=None, every=1, **kwargs):
        ...
        if not engine.has_event_handler(self.iteration_completed, Events.ITERATION_COMPLETED):
            engine.add_event_handler(Events.ITERATION_COMPLETED(every=every), self.iteration_completed)
